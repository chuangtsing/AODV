...
/*
	...
	{
	sprintf(buf, "/system/bin/insmod /data/data/org.span/bin/kaodv.ko ifname=%s &>/dev/null", ifname);
	fprintf(stderr, "%s/kaodv.ko", currentPath);
	}
    else if (stat("./kaodv.o", &st) == 0)
	sprintf(buf, "/system/bin/insmod /data/data/org.span/bin/kaodv.o ifname=%s &>/dev/null", ifname);
    else
	sprintf(buf, "/system/xbin/modprobe kaodv ifname=%s &>/dev/null", ifname);
    execl("/system/bin/sh", "sh", "-c", "insmod /data/data/org.span/bin/kaodv.ko &>/dev/null", (char *)NULL);
    if (system(buf) == -1) {
...
void remove_modules(void)
{
	...
	ret = system("/system/bin/rmmod kaodv &>/dev/null");
